		...
		for _, vh := range routeConfig.VirtualHosts {
			for _, route := range vh.Routes {
				if strings.Contains(route.Name, internalapi.MCPMainHTTPRoutePrefix) {
					...
					// Remove the JWT authn filter from the well-known and backend routes.
					// TODO: remove this step once the SecurityPolicy can target the MCP proxy route rule only.
					if _, ok := route.TypedPerFilterConfig[jwtAuthnFilterName]; ok {
						s.log.Info("removing JWT authn filter from well-known and backend routes", "route", route.Name)
						delete(route.TypedPerFilterConfig, jwtAuthnFilterName)
					}
				}
			}
		}
